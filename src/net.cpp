...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "WLC " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"115.28.214.110", "115.28.214.110"},
    {"121.42.186.227", "121.42.186.227"},
    {NULL,NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. WLC is probably already running."), addrBind.ToString().c_str());
        else
